...
class warrantyXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        product_name = workbook.add_format({'font_size': 12, 'align': 'vjustify', 'bold': True, 'bg_color': '#9bd8e0'})
        table_contents = workbook.add_format({'font_size': 10, 'align': 'vjustify','bold': True,})
        sheet = workbook.add_worksheet('Warranty Report')
        # print("print data", data)

        sheet.set_column(6, 76, 30)
        sheet.set_column(6, 4, 5)
        sheet.set_column(1, 5, 15)
        sheet.set_column(1, 6, 15)
        sheet.set_column(1, 7, 15)
        sheet.set_column(1, 8, 15)

        sheet.write(4, 5, 'Warranty Report 2020', main_heading)
        sheet.write(6, 3, 'SL NO.', table_heading)
